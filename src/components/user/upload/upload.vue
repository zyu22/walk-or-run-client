<template>
  <div class="mx-auto max-w-4xl p-6">
    <h1 class="mb-8 text-2xl font-bold">파일 업로드</h1>

    <!-- CSV 파일 업로드 폼 -->
    <div class="mb-8 rounded-lg bg-white p-6 shadow-sm">
      <h2 class="mb-4 text-lg font-medium">CSV 파일 업로드</h2>

      <form @submit.prevent="handleCsvUpload" class="space-y-4">
        <div class="flex flex-col space-y-2">
          <label for="csvFile" class="text-sm text-gray-600"> CSV 파일을 선택해주세요 </label>
          <div class="flex items-center space-x-4">
            <input
              ...
              ref="fileInput"
              accept=".csv"
              class="flex-1 rounded-lg border border-gray-200 px-4 py-2 focus:outline-none focus:ring-2 focus:ring-orange-500"
    ...
    <div class="rounded-lg bg-white p-6 shadow-sm">
      <h2 class="mb-4 text-lg font-medium">업로드 이력</h2>
...
const handleCsvUpload = async (event) => {
  ...
  try {
    const response = await api.post(`/user/${userStore.userId}/record/upload`, formData, {
      headers: {
